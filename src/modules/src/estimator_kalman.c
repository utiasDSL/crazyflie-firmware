...
static float log_HPHR_chi = 0.0f;
...
static void stateEstimatorScalarUpdate(arm_matrix_instance_f32 *Hm, float error, float stdMeasNoise)
{
  ...
  log_HPHR_chi = HPHR;
  log_errAbs = err_abs;
  log_h1 = Hm->pData[0]; log_h2 = Hm->pData[1]; log_h3 = Hm->pData[2];
    ...
    if (tdoaCount >= 100)
  {
    ...
    if ((d0 != 0.0f) && (d1 != 0.0f) && (enable_UWB)) {
        ...
        for (int iter = 0; iter < 3; iter++){
            ...
            // float P_cTd[9][9] = {0};
            // arm_matrix_instance_f32 P_cTm = {9, 9, (float *)P_cTd};
            // mat_trans(&Pc_m, &P_cTm);
            // float P_test[9][9] = {0};
            // arm_matrix_instance_f32 P_testm ={9, 9, (float *)P_test};
            // mat_mult(&Pc_m, &P_cTm, &P_testm);           //  P_test = P
            //----------------------------------------------//
        ...
        for (int i=0; i<STATE_DIM; i++) { tmpNN1d[i][i] = 1.0f - tmpNN1d[i][i]; } //  I-KH
        mat_mult(&tmpNN1m, &P_w_m, &Pm);      // Pm = (I-KH)*P_w_m
